/**
 * AscendingMinorLadder - Ascending scale degree patterns in minor scale.
 *
...
export class AscendingMinorLadder extends BaseExercise {
  constructor() {
    super();
    this.name = 'Ascending Minor Ladder';
    this.description = 'Ascending patterns through the minor scale';
  ...
  generatePhases(scaleManager) {
    ...
    for (let ladder = 1; ladder <= ladderCount; ladder++) {
      ...
      const degreeIndices = AscendingMinorLadder.generateLadderPattern(ladder);
    ...
    for (let i = 1; i <= ladderCount; i++) {
      ladderLabels.push(AscendingMinorLadder.generateLadderPattern(i).map(d => d + 1).join('-'));
    }
