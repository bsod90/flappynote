/**
 * DescendingMinorLadder - Descending scale degree patterns in minor scale.
 *
...
export class DescendingMinorLadder extends BaseExercise {
  constructor() {
    super();
    this.name = 'Descending Minor Ladder';
    this.description = 'Descending patterns through the minor scale';
  ...
  generatePhases(scaleManager) {
    ...
    // degrees array has indices 0-7 for an 8-note scale (Do-Re-Me-Fa-Sol-Le-Te-Do)
    ...
    for (let ladder = 1; ladder <= ladderCount; ladder++) {
      ...
      const degreeIndices = DescendingMinorLadder.generateDescendingLadderPattern(ladder, octaveIndex);
    ...
    for (let i = 1; i <= ladderCount; i++) {
      const pattern = DescendingMinorLadder.generateDescendingLadderPattern(i, octaveIndex);
