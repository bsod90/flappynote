/**
 * ScaleDegreePatternExercise - Ascending/descending scale degree patterns.
 *
 * Generates one continuous sequence of "ladders":
...
export class ScaleDegreePatternExercise {
  constructor() {
    this.name = 'Scale Degree Patterns';
    this.description = 'Ascending/descending patterns through the scale';
    this.sustainDuration = 200; // 0.2 seconds
  }
  ...
  generatePhases(scaleManager) {
    ...
    for (let ladder = 1; ladder <= ladderCount; ladder++) {
      const degreeIndices = ScaleDegreePatternExercise.generateLadderPattern(ladder);
    ...
    for (let i = 1; i <= ladderCount; i++) {
      ladderLabels.push(ScaleDegreePatternExercise.generateLadderPattern(i).map(d => d + 1).join('-'));
    }
